...
$(function(){
  ...
  const contentsTitle = $("#main-contents-title").html();
 
  //현재 선택되어있는 class들을 없앰
  $(".nav-side-item-active a")
    .addClass("nav-side-item-a")
    .parent()
    .removeClass("nav-side-item-active")
  ...
  $(".nav-side-container")
    .children()
    .each((index, el) => {
      if ($(el).find('a').html() == contentsTitle) {
        //nav-side와 contents-title이 같을 경우 class 추가
        $(el)
          .removeClass("nav-side-item")
          .addClass("nav-side-item-active")
          .children()
          .eq(0)
          .removeClass("nav-side-item-a");
      }
    });
