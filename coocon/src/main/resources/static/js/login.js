$(function() {
	...
	$('#register-form').submit(function(e){
		...
		if(email == '' || username == '' || deptname == '' || password == '' || password_ck == ''){
			...
			// 영문+숫자조합 최소 8자리 최대 20자리(특수기호 가능)
			$('#pwPolicyModal').toggle();
		} else {
			// 정상 회원가입
			$.post('api/signUp', $(e.target).serialize(), function(){
				$('#signUpModal').toggle();
	...
	$('#pwInit-form').submit(function(e){
		...
		$.post('api/getStatus', $(e.target).serialize(), function(data){
			if(data.status == '승인대기'){
				$('#findPwModal').toggle();
				$('#failPwInitResetModal').toggle();
			} else {
				$.post('api/initPw', $(e.target).serialize(), function(){
					$('#findPwModal').toggle();
   ...
   // Close 비밀번호 정책 오류 form
	$('#pwPolicyModalExp').click(function(){
		$('#pwPolicyModal').toggle();
	})
	$('#pwPolicyModalClose').click(function(){
		$('#pwPolicyModal').toggle();
	})
	...
	// Close 승인대기 상태 비밀번호 초기화 실패 form
	$('#failPwInitResetModalExp').click(function(){
		$('#failPwInitResetModal').toggle();
	})
	$('#failPwInitResetModalClose').click(function(){
		$('#failPwInitResetModal').toggle();
	})
